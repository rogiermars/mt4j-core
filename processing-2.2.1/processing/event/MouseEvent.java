...
//  }
  ...
  public java.awt.event.MouseEvent toAwtMouseEvent(Component source) {
	int id = toAwtMouseEventID(getAction());
	long when = getMillis();
	int modifiers = getModifiers();
	int x = getX();
	int y = getY();
	int clickCount = getCount();
	boolean popupTrigger = false;
	int button = toAwtMouseEventButton(getButton());
	return new java.awt.event.MouseEvent(source, id, when, modifiers, x, y, clickCount, popupTrigger, button);
  }


	private static int toAwtMouseEventButton(int button) {
		switch (button) {
  ...
  private static int toAwtMouseEventID(int action) {
	switch (action) {
    case CLICK:
