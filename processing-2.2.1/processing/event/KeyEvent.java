...
public class KeyEvent extends Event {
  ...
  public java.awt.event.KeyEvent toAwtKeyEvent(Component source) {
	int id = toAwtKeyEventID(getAction());
	long when = getMillis();
	int modifiers = getModifiers();
	int keyCode = getKeyCode();
	char keyChar = getKey();
	return new java.awt.event.KeyEvent(source, id, when, modifiers, keyCode, keyChar);
  }

  private static int toAwtKeyEventID(int action) {
	switch (action) {
    case PRESS:
