...
//        writer.write(null, new IIOImage(bimage, null, null), param);
  ...
  public void delete() {
    ...
    /**
	 * Efficient method of drawing an image's pixels directly to this surface. No variations are employed, meaning that
	 * any scale, tint, or imageMode settings will be ignored.
	 */
	public void setMask(int x, int y, PImage src) {
		set(x, y, src, true);
	}
	 ...
	 * @param src
	 * @param mask
	 *            Whether src is a black and white mask image
	 */
	protected void set(int x, int y, PImage src, boolean mask) {
		int sx = 0;
		int sy = 0;
		int sw = src.width;
		int sh = src.height;
		...
		if ((x + sw) > this.width) { // off right edge
			sw = this.width - x;
		}
		if ((y + sh) > this.height) { // off bottom edge
			sh = this.height - y;
		}
		...
		if (mask) {
			this.setMaskImpl(x, y, sx, sy, sw, sh, src);
		} else {
	...
	/**
	 * MASK IMPLEMENTATION. Internal function to actually handle setting a block of pixels that has already been
	 * properly cropped from the image to a valid region.
	 */
	protected void setMaskImpl(int dx, int dy, int sx, int sy, int sw, int sh, PImage src) {
		int srcOffset = (sy * src.width) + sx;
		...
		for (int y = sy; y < (sy + sh); y++) {
			for (int i = 0; i < sw; i++) {
				...
				// BGRA
				int sB = src.pixels[srcPos] & 0xff;
				// int sG = (src.pixels[srcPos] >> 8) & 0xff;
				// int sR = (src.pixels[srcPos] >> 16) & 0xff;
				// int sA = (src.pixels[srcPos] >> 24) & 0xff;
				
				int dB = this.pixels[srcPos] & 0xff;
				int dG = (this.pixels[srcPos] >> 8) & 0xff;
				int dR = (this.pixels[srcPos] >> 16) & 0xff;
				int dA = (this.pixels[srcPos] >> 24) & 0xff;
				
				int fB = dB;
				int fG = dG;
				int fR = dR;
				int fA = dA; // 0=alpha, 255=opaque
				
//				int pixel = (fB) | (fG << 8) | (fR << 16) | (fA << 24);
//				int pixel = (255-sB) | (0 << 8) | (0 << 16) | (255 << 24);
				
				 int srcAlpha = (src.pixels[srcPos] & 0xff) << 24;
				 int dstAlpha = this.pixels[dstPos] & 0xff000000;
				 int pixel = this.pixels[dstPos];
				 // pixel = Math.max(dstAlpha, srcAlpha) | (pixel & 0xffffff);
				 pixel = srcAlpha | (pixel & 0xffffff);
				 pixel = dstAlpha & pixel;
				this.pixels[dstPos] = pixel;
			}
			srcOffset += src.width;
			dstOffset += this.width;
		}
		this.updatePixels(sx, sy, sx + sw, sy + sh);
	}
